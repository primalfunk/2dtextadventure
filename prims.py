...
class Prims:
    ...
    def generate_mst(self):
        ...
        self.distances[start_room] = 0
        queue = [(0, start_room)]

        while queue:
            _, current_room = heapq.heappop(queue)
            if current_room not in self.mst:
                self.mst.add(current_room)
                for neighbor in self.get_neighbors(current_room):
                    distance = self.distance(current_room, neighbor)
                    self.update_distance_in_queue(queue, neighbor, distance)

    def get_neighbors(self, room):
        neighbors = []
        for potential_neighbor in self.rooms:
            if potential_neighbor != room and potential_neighbor not in self.mst:
    ...
    def calculate_direction(self, room1, room2):
        ...
        if dx > 0:
            ...
            return "south"
        else: # dx and dy are both 0
            print("Warning: two rooms have the same coordinates.")
            return None
